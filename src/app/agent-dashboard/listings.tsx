...
import { ChevronLeft, Plus, Eye, Users, Circle } from 'lucide-react-native';
...
export default function AgentListingsScreen() {
  ...
  const filteredListings = activeTab === 'all'
    ? myListings
    : myListings.filter(p => p.status === activeTab);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={() => router.push('/add-listing')} activeOpacity={0.7}>
      ...
      <View style={styles.tabsWrapper}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabsContainer}>
          <TouchableOpacity style={[styles.tab, activeTab === 'all' && styles.tabActive]} onPress={() => setActiveTab('all')}>
            <Text style={[styles.tabText, activeTab === 'all' && styles.tabTextActive]}>All ({myListings.length})</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity style={[styles.tab, activeTab === 'rented' && styles.tabActive]} onPress={() => setActiveTab('rented')}>
            <Text style={[styles.tabText, activeTab === 'rented' && styles.tabTextActive]}>
              Rented ({myListings.filter(p => p.status === 'rented').length})
            </Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.tab, activeTab === 'pending_review' && styles.tabActive]} onPress={() => setActiveTab('pending_review')}>
            <Text style={[styles.tabText, activeTab === 'pending_review' && styles.tabTextActive]}>
              Pending ({myListings.filter(p => p.status === 'pending_review').length})
            </Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  propertyInfo: { padding: 16, gap: 8 },
  statusBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, alignSelf: 'flex-start',
  },
  statusText: { fontSize: 12, fontWeight: '700' },
  propertyTitle: { fontSize: 16, fontWeight: '700', color: COLORS.text, lineHeight: 22 },
  propertyLocation: { fontSize: 13, color: COLORS.textSecondary },
  propertyRent: { fontSize: 18, fontWeight: '800', color: COLORS.primary, marginTop: 4 },
  propertyStats: { flexDirection: 'row', gap: 20, marginTop: 8 },
