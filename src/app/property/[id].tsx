...
import {
  ...
  MessageCircle, Calendar, Calculator, Images, X, Star, Navigation,
} from 'lucide-react-native';
import { useApp } from '@/context/AppContext';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import PropertyVideo from '@/components/PropertyVideo';
import MapModal from '@/components/MapModal';
import { staticMapUrl } from '@/config/mapbox';
    ...
    <>
      ...
      <View style={styles.specRow}>
        <View style={styles.specItem}>
          <BedDouble size={18} color={COLORS.text} strokeWidth={1.8} />
          <Text style={styles.specText}>{property.bedrooms} bed</Text>
        </View>
        <View style={styles.specItem}>
          <Bath size={18} color={COLORS.text} strokeWidth={1.8} />
          <Text style={styles.specText}>{property.bathrooms} bath</Text>
        </View>
        {(property.verifiedPhone || property.verifiedId) && (
          <View style={styles.specItem}>
            <ShieldCheck size={18} color={COLORS.primary} strokeWidth={1.8} />
            <Text style={styles.specText}>Verified</Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.locationCard}>
        <View style={styles.locationCardRow}>
          ...
          <Text style={styles.locationCardText}>
            {property.address}, {property.neighbourhood}, {property.city}
          </Text>
        </View>
        <TouchableOpacity style={styles.mapPreview} onPress={() => setMapOpen(true)} activeOpacity={0.9}>
          <Image
            source={{ uri: staticMapUrl(property.lng, property.lat) }}
            style={styles.mapPreviewImg}
            resizeMode="cover"
          />
          <View style={styles.mapOpenChip}>
            <Navigation size={13} color={COLORS.primary} strokeWidth={2.2} />
      ...
      <View style={styles.ownerCard}>
        ...
        <View style={styles.ownerInfo}>
          ...
          <View style={styles.ownerMetaRow}>
            <Star size={12} color={COLORS.secondary} fill={COLORS.secondary} strokeWidth={0} />
            <Text style={styles.ownerRating}>{property.ownerRating.toFixed(1)} rating</Text>
          </View>
        </View>
        <TouchableOpacity style={styles.ownerMsgBtn} onPress={handleContactOwner} activeOpacity={0.85}>
      ...
      >
        {isDesktop ? (
          <View style={styles.twoCol}>
            ...
            <View style={[styles.rightCol, { position: 'sticky', top: 20 } as any]}>
              {costCardEl}
...
const styles = StyleSheet.create({
  ...
  specItem: { flexDirection: 'row', alignItems: 'center', gap: 7 },
  specText: { fontSize: 14.5, color: COLORS.text, fontWeight: '500' },
  backBtn: {
  ...
  typePill: {
    ...
    backgroundColor: COLORS.primaryLight, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 20,
  },
  typePillText: { fontSize: 12, fontWeight: '700', color: COLORS.primary },
  metaTimeRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  metaTimeText: { fontSize: 12, color: COLORS.textSecondary, fontWeight: '500' },
  ...
  trustChip: {
    ...
    backgroundColor: COLORS.primaryLight, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 10,
  },
  trustChipText: { fontSize: 12, fontWeight: '700', color: COLORS.primary },
  factsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 16 },
  factItem: {
    flexDirection: 'row', alignItems: 'center', gap: 10,
    flexGrow: 1, flexBasis: '45%', minWidth: 130,
    backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border,
    borderRadius: 14, padding: 12, borderCurve: 'continuous',
  },
  factIconWrap: {
    width: 36, height: 36, borderRadius: 10, backgroundColor: COLORS.primaryLight,
    alignItems: 'center', justifyContent: 'center',
  },
  factTextWrap: { flex: 1, minWidth: 0 },
  factValue: { fontSize: 14, fontWeight: '800', color: COLORS.text },
  factLabel: { fontSize: 11, color: COLORS.textSecondary, fontWeight: '500' },
  costCard: {
  ...
  locationCard: {
    backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border,
    borderRadius: 14, padding: 14, gap: 12, borderCurve: 'continuous',
  },
  locationCardRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 },
  locationCardText: { flex: 1, fontSize: 13, color: COLORS.text, lineHeight: 19 },
  mapPreview: {
    height: 170, borderRadius: 12, overflow: 'hidden',
    backgroundColor: '#ECEEF0', position: 'relative',
  },
  mapPreviewImg: { width: '100%', height: '100%' },
  mapOpenChip: {
  ...
  ownerAvatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: COLORS.border },
  ownerInfo: { flex: 1, gap: 3 },
  ownerName: { fontSize: 15, fontWeight: '700', color: COLORS.text },
  ownerMetaRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  ownerRating: { fontSize: 13, color: COLORS.textSecondary, fontWeight: '600' },
  ownerMsgBtn: {
